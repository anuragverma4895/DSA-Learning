...
void insert(int arr[], int* n, int key) {
    ...
    // Fix the max heap property by comparing with the parent
    int i = *n - 1;
...
// Function to remove the element with the highest priority (i.e., root of the heap)
...
int main() {
    ...
    printf("Max element (priority): %d\n", peekMax(arr));

    printf("Extracted max: %d\n", extractMax(arr, &n));
    display(arr, n);  // Display the heap after extracting max
